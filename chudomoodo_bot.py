...
- принимает короткие радости;
- очищает и маскирует мат (расширенный словарь);
- НЕ записывает в радости нейтральные фразы типа «не знаю, что написать»;
- сохраняет радости в SQLite;
- ЕЖЕДНЕВНО:
    - в 18:00 — напоминание, если за день не было ни одной радости;
    - в 21:00 — отчёт за день (без фраз «не знаю...» и т.п.);
...
- ачивки за количество радостей и стрики по дням (теперь максимум одно сообщение);
- статистика по /stats;
- письма себе в будущее по /letter (с отменой через /cancel);
- бот реагирует на приветствия (расширенный словарь) и сам приветствует по /start.
"""
...
# Расширенный список мата (можно дополнять)
BAD_WORDS = [
    "хуй", "хуи", "хер", "пизда", "ебать", "ебан", "сука", "бляд", "бля",
    "хуйн", "хуев", "хуёв", "хуеплет", "хуеплёт", "хуеплета", "хуесос",
    "пидор", "пидар", "пидорас", "пидарас", "мудак", "долбоеб", "долбоёб",
    "еблан", "ебло", "ебана", "ебанутая", "ебанутый", "ебануто", "хуесоска",
    "пиздец", "пиздос", "пизданутый", "пизданутая", "пиздануло", "охуел",
    "охуела", "охуели", "охуенный", "охуенная", "охренеть", "охренел",
    "иди на хуй", "иди нахуй", "иди нахер", "иди нахрен",
    "нахер", "нахрен", "нахуй", "ебучий", "ебучая",
    "ебаный", "ебаная", "ебаное", "ебаные", "мразь", "скотина", "сукин сын",
    "шалава", "проститутка", "шлюха", "шлюшка", "тварь", "урод", "говно",
    "говнюк", "дерьмо", "сволочь", "сука блять", "сука, блять",
]
...
# Приветствия — расширенный список (можно дополнять)
GREETING_PATTERNS = [
    "привет", "приветик", "приветики", "прив", "приветь",
    "хай", "хайка", "ку", "здорово", "здарова", "здорова",
    "здравствуйте", "здравствуй",
    "добрый день", "добрый вечер", "доброе утро",
    "доброго дня", "доброго вечера", "доброго утра",
    "хеллоу", "хелло", "hello", "hi", "hey",
    "йоу", "йо", "салам", "салют", "чао",
    "добрейшего утречка", "добрейшего", "доброго",
    "привет бот", "привет, бот", "привет дневник", "привет, дневник",
    "эй", "эй ты", "привет, это я",
    "я пришла", "я пришел", "я вернулась", "я вернулся",
]

GREETING_RESPONSES = [
    "Привет. Как твой день? Расскажешь что-нибудь хорошее, даже если оно совсем маленькое?",
    "Привет, я тут. Можешь написать одну радость за сегодня — даже если это просто вкусный чай.",
...
SAD_RESPONSES = [
    ...
    "Иногда единственное хорошее — это то, что день закончился. А если вдруг всплывёт что-то потеплее — я здесь.",
    ...
    "Можем пока просто зафиксировать, что ты его выдержала. А к радостям вернёмся, когда появится хотя бы немного ресурса.",

    "Когда вокруг всё кажется серым, очень сложно что-то чувствовать.\n\n"
    "Если позже поймаешь момент тепла — даже совсем крошечный — я с радостью его сохраню для тебя.",

    "Звучит так, будто этот день больше забрал, чем дал.\n\n"
    "Можно пока просто быть. А когда появится хоть один тихий приятный момент — напиши о нём.",
]

# Ответы на усталость
TIRED_RESPONSES = [
    "Похоже, день тебя основательно выжал.\n\n"
    "Это не про слабость, а про то, что ты много на себе несёшь. Если вспомнишь момент, где стало хоть немного легче — напиши.",
...
ANXIETY_RESPONSES = [
    ...
    "Иногда помогает опереться на что-то очень простое: чай, прогулка, сообщение от близкого человека. Если хочешь, напиши об этом моменте.",
    ...
    "Звучит так, будто внутри шумно и неспокойно.\n\n"
    "Давай не требовать от себя полной уверенности, но попробуем найти маленькое «я всё же справилась хотя бы здесь».",
]

# Ответы на "не знаю, что написать"
NO_JOY_RESPONSES = [
    ...
    "Окей, оставим этот момент таким, как есть. Если за день мелькнёт хоть маленький приятный эпизод — я здесь.",
...
JOY_RESPONSES = [
    ...
    "Спрятала эту радость здесь, как маленький тёплый камушек в кармане.",
...
def get_first_joy_date(chat_id: int) -> Optional[date]:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT MIN(substr(created_at, 1, 10)) FROM joys WHERE chat_id = ?",
        (chat_id,),
    )
    row = cur.fetchone()
    conn.close()
...
def is_greeting_message(text: str) -> bool:
    ...
    for p in GREETING_PATTERNS:
        p_norm = normalize_text_for_match(p)
...
# --------------------------
# Ачивки — максимум одно сообщение
# --------------------------
...
def check_and_send_achievements(chat_id: int):
    ...
    if total == 1:
        ...
        candidate_messages.append(add_emoji_prefix(random.choice(options)))
    elif total == 7:
        options = [
    ...
    if candidate_messages:
        send_message(chat_id, random.choice(candidate_messages))


# --------------------------
# Ритуал «3 маленькие радости»
# (оставляем, но больше не шлём его сразу вместе с грустными ответами)
# --------------------------
...
def send_weekly_human_summary(chat_id: int):
    ...
    header = "Собрала для тебя небольшой обзор недели — вот хорошие моменты, которые были рядом с тобой:"
    body = "\n".join(lines)
...
def send_stats(chat_id: int):
    ...
    msg = (
        ...
        f"• Сейчас у тебя есть своя серия дней, где ты не забываешь про себя.\n"
                    ...
                    try:
                        if not has_joy_for_date(user_id, today):
                            ...
                            send_message(
                                user_id,
                                f"{emo} Уже вечер.\n"
                                "Скорее всего, в этом дне было хоть что-то чуть приятнее остального. "
                                "Если захочешь, напиши мне об этом — я сохраню."
                            )
                    except Exception as e:
...
def send_daily_report_for_user(chat_id: int):
    ...
    # фильтруем фразы "не знаю, что написать" и т.п.
    filtered = []
    for created_at, text in joys:
    ...
    header = "Давай посмотрим, что хорошего ты успела заметить в этом дне:"
    body = "\n".join(lines)
    tail = (
...
def process_incoming_message(update: dict):
    ...
    # Приветствия — просто ответ, без записи радости
    ...
    # Нейтральные "не знаю, что написать" — не сохраняем в БД
