...
BAD_WORDS = [
    ...
    "пиздёж", "пиздеж", "ебать", "ебаный", "ёбаный", "ебанутый", "ёбанутый", "ебанутая",
    "ёбанутая", "ебаные", "ёбаные", "ебаного", "ебаному", "ебану", "ебалась", "ебался",
    "ебёмся", "ебемся", "ебусь", "ебешься", "ебешь", "ебёт", "ебет", "ебучий", "ебучая",
    "ебучие", "ебанулась", "ебанулся", "выебать", "выебал", "выебала", "выебали",
    "проебать", "проебал", "проебала", "проебали", "проебали", "проебано",
    "проёб", "проеб", "проёмы", "проебы", "проебываю", "проёбываю",
    "сука", "суки", "сукин", "сукина", "сукины", "сука блять", "сука, блять",
    "сука,блять", "сучка", "сучки", "сучара", "сучары", "суходрочка",
    "блядь", "блять", "блядская", "блядский", "блядские", "бляди", "блядина",
    "блядище", "блядовать", "блядуха", "блядство", "бляха", "бляха-муха",
    "ебала", "ебало", "ебальник", "ебальце", "ебануло", "ебнуло",
    "манда", "манда", "мандавошка", "мандавошки", "мандраж", "мандить",
    "жопа", "жопы", "жопка", "жопки", "жопень", "жопищще", "жопища", "жопный",
    "вжопу", "в жопу", "ожопился", "жопиться", "жопиться",
    "говно", "говна", "говном", "говнистый", "говняный", "говнюк", "говнюки",
    "говнючий", "говнокод", "говнокоуч", "говносайт", "говнопроект",
    "дерьмо", "дерьма", "дерьмовый", "дерьмовая", "дерьмовые",
    "мудак", "мудаки", "мудачка", "мудачье", "мудацкий",
    "козёл", "козел", "козлина", "козлище", "козлиный",
    "идиот", "идиоты", "идиотка", "идиотский",
    "дебил", "дебилы", "дебильный", "дебильная",
    "ебанат", "ёбанат", "долбоёб", "долбоеб", "долбоёбы", "долбоебы",
    "долбанутый", "долбанутая", "еблан", "ёблан", "ебланы",
    "пидор", "пидоры", "пидорас", "пидорасы", "пидорский",
    "пидрила", "пидрилы", "гондон", "гандон", "гандоны",
    "мразь", "мрази", "мразота", "ублюдок", "ублюдки", "ублюдки",
    "сука ты", "сука же", "сука бля", "еб твою мать", "ёб твою мать",
    "твою мать", "твоюж мать", "твою ж мать",
    "нахер", "нахрен", "нахуй", "нах", "нахуй-то", "на хрен", "на хуй",
    "иди нахуй", "иди на хуй", "пошел нахуй", "пошёл нахуй", "пошел на хуй",
    "пошёл на хуй", "пошла нахуй", "пошла на хуй",
    "срать", "сру", "срал", "срала", "насрать", "насрал", "насрала",
    "обосрался", "обосралась", "обосраться",
    "херня", "херни", "херню", "херовый", "херово", "хуёвина", "хуевина",
    "залупа", "залупы", "залупка", "залупки",
    "ебля", "ёбля", "трахаюсь", "трахаешься", "трахаться", "трахнул", "трахнула",
    "трахнули", "перетрахать", "натрахаться",
    "шалава", "шалавы", "проститутка", "проститутки", "простолюдина",
    "стерва", "стервы", "мерзавец", "мерзавцы",
    "сучий потрох", "сучий сын", "сукин сын", "сукина дочь",
    "хреначить", "хреначу", "хуячу", "хуярить", "хуярю", "хуярим",
    "ебар", "ебарь", "ебаник", "ебаник", "выебистый", "выёбистый",
    "выёбываться", "выебываться", "заебать", "заебал", "заебали",
    "задолбал", "задолбали", "задрачивать", "задрачиваю",
    "охуел", "охуела", "охуели", "охуенно", "охуенный", "охуенная",
    "охренеть", "обалдеть"  # список сильно расширен; при желании можно дополнить
]
...
ANXIETY_PATTERNS = [
    ...
    "не знаю, что будет дальше и страшно",
]

# Нейтральные/растерянные фразы "не знаю, что написать" (расширено)
NO_JOY_PATTERNS = [
    ...
    "ничего хорошего не приходит в голову",
    "пока не придумала",
    "пока не придумал",
    "пока нечего сказать",
...
# Расширенный список приветствий (нормализованные формы, без знаков препинания)
GREETING_PATTERNS = [
    "привет", "приветик", "приветики", "прив", "привэт", "приветствую",
    "здравствуй", "здравствуйте", "здрасте", "здрасте всем", "доброго дня",
    "добрый день", "добрый вечер", "доброе утро", "доброй ночи",
    "хай", "хай там", "хей", "хеллоу", "хелло", "йоу",
    "ку", "ку ку", "кку", "здарова", "здорова", "здорово",
    "салют", "добрый", "добрейший вечерочек", "доброты", "добра",
    "добрый денек", "добрый денёк", "добрейший день",
    "добрейший", "приветики солнце", "привет солнце",
    "приветик солнышко", "привет солнышко",
    "приветик зайка", "привет зайка",
    "добро пожаловать", "рада видеть", "рад видеть",
    "хэй", "хэллоу", "hello", "hi", "hey",
    "добрый вам день", "добрый вам вечер", "доброе вам утро",
    "привет мой дневник", "привет дневник", "привет бот",
    "приветик бот", "здравствуй бот", "здравствуй дневник",
    "привет ещё раз", "привет еще раз", "снова привет",
    "опять привет", "добрый вечерочек", "добренький вечерок",
    "добрейшего", "доброго времени суток", "доброго времени",
    "доброго утра", "холла", "hola", "бонжур", "бонжорно",
    "салютик",
]

GREETING_RESPONSES = [
    "Привет. Как твой день? Если хочешь, расскажи что-нибудь хорошее, даже если это совсем мелочь.",
    "Привет, я тут и слушаю. Можешь скинуть одну маленькую радость за сегодня — даже если это просто вкусный чай.",
    "Рада тебя видеть здесь. Давай вместе отметим что-нибудь тёплое или спокойное из этого дня.",
    "Привет. Здесь можно выдохнуть и вспомнить момент, где было хоть чуть-чуть легче или уютнее.",
    "Хей. Если день вышел тяжёлым, можно начать с чего-то очень простого: еда, пауза, сообщение от близкого.",
]
...
SAD_RESPONSES = [
    ...
    "Если позже всплывёт момент, где стало хотя бы чуть легче, напиши — я бережно его сохраню.",

    "Понимаю, что сегодня могло быть тяжко.\n\n"
    "Иногда единственное хорошее — это то, что день закончился. А если вдруг появится что-то хоть немного мягче и светлее — я рядом.",

    "Бывает, что день совсем не радует. Так тоже можно жить какое-то время.\n\n"
    "Если захочешь, попробуем найти одну маленькую опору: паузу, сообщение, тёплый напиток.",

    "Слышу, что внутри много тяжести.\n\n"
    "Не нужно притворяться, что всё ок. Когда появится хоть один момент, который не про «ужас», а про «стало немного полегче» — расскажи мне о нём.",

    "Этот день явно не из простых.\n\n"
    "Можем просто зафиксировать, что ты его прошла. А к радостям вернёмся, когда появится хоть капля ресурса.",

    "Кажется, в этом дне было много тоски и мало опоры.\n\n"
    "Не обязана прямо сейчас вытаскивать что-то хорошее. Я останусь здесь и подожду любой маленький тёплый момент.",
]

TIRED_RESPONSES = [
    "Похоже, день тебя основательно выжал.\n\n"
    "Это не про слабость, а про то, что ты слишком много тащишь. Если вспомнишь момент, где стало хотя бы на полтона легче — напиши.",
    ...
    "Если будет желание, отметим хотя бы один момент, который не был таким тяжёлым, как остальное.",
]
...
ANXIETY_RESPONSES = [
    ...
    "Волнение — нормальная реакция, когда вокруг много неопределённости.\n\n"
    "Если ты вспомнишь, где сегодня тебе удалось хоть на секунду выдохнуть — расскажи мне, я это запомню.",

    "Звучит так, будто внутри сейчас очень громко.\n\n"
    "Давай не будем требовать от себя полного спокойствия, но попробуем заметить маленькое «я справилась хотя бы здесь».",
]
...
JOY_RESPONSES = [
    ...
    "Положила эту радость рядом с другими тёплыми вещами, которые ты уже заметила.",
    "Сохранила как маленькое напоминание: даже сегодня нашлось место для чего-то хорошего.",
    "Записала. Пусть это станет частью мягкого отношения к себе.",
    "Оставила этот момент в дневнике, как точку, где тебе было немного теплее.",
]

NO_JOY_RESPONSES = [
    "Бывает, что день кажется пустым, а слова не находятся.\n"
    "Можно ничего не выжимать из себя. Если позже всплывёт что-то тёплое — просто напиши.",
    "Нормально не знать, что написать. Иногда за этим «не знаю» прячутся усталость, грусть или растерянность.\n"
    "Если захочешь, мы можем заметить хотя бы одну маленькую опору в дне.",
    "Окей, оставим этот момент таким, как есть.\n"
    "Если за день мелькнёт хоть малюсенький приятный эпизод — чаёк, сообщение, улыбка — я здесь и сохраню его.",
    "Иногда мозг просто устал и не выдаёт ничего. Это не делает тебя хуже.\n"
    "Если что-то всплывёт — от самого простого до совсем маленького — напиши, я аккуратно его сохраню.",
    "Пусть сегодня будет пауза. Ты не обязана быть «интересной» или «позитивной».\n"
    "Можешь вернуться и поделиться, когда почувствуешь хоть крошечный светлый момент.",
]
...
def is_greeting_message(text: str) -> bool:
    lower = normalize_text_for_match(text)
    # реагируем, если фраза начинается с приветствия
    return any(lower == p or lower.startswith(p + " ") for p in GREETING_PATTERNS)
...
def check_and_send_achievements(chat_id: int):
    ...
    # отправляем не больше одного сообщения за раз
    if messages:
        combined = "\n\n".join(messages)
...
def maybe_offer_ritual(chat_id: int):
    # Оставлено как задел: сейчас не шлёт отдельные сообщения,
    # чтобы не было по 2–3 ответа от бота подряд.
    _ = get_sad_count_last_days(chat_id, SAD_RITUAL_DAYS)
    return
...
def start_small_joy_dialog(chat_id: int):
    send_message(
        ...
        add_emoji_prefix(
            ...
            "Если захочешь, можешь написать о моменте, который был чуть менее тяжёлым: пауза, еда, кто-то поддержал, музыка.\n"
...
def handle_small_joy_reply(chat_id: int, text: str):
    if is_cancel_message(text):
        ...
        send_message(
            chat_id,
            add_emoji_prefix(
                "Хорошо, без маленькой опоры сейчас. Если позже вспомнишь что-то, что ощущалось чуть мягче — можешь просто написать."
            )
        )
        return
...
# --------------------------
# Помощник для фильтра отчёта
# --------------------------

def is_joy_suitable_for_report(text: str) -> bool:
    """
    В ежедневный отчёт попадают только записи о событиях и эмоциях,
    без фраз «не знаю», без мата (заменённого звёздочками) и пустых общих фраз.
    """
    norm = normalize_text_for_match(text)

    # исключаем явно растерянные «не знаю, что написать» и похожие
    if is_no_joy_message(text) or "не знаю" in norm or "я не знаю" in norm:
        return False

    # исключаем записи, где остались маскированные ругательства
    if re.search(r"\*{2,}", text):
        return False

    # слишком короткие общие фразы, которые мало что говорят о событии
    if len(norm) < 5:
        return False
...
def process_incoming_message(update: dict):
    ...
    if stripped.startswith("/cancel"):
        ...
        if state in ("await_letter_period", "await_letter_text"):
            ...
            send_message(
                chat_id,
                add_emoji_prefix(
                    "Хорошо, без маленькой опоры сейчас. Если позже вспомнишь что-то, что ощущалось хоть немного мягче — можешь просто написать."
                )
            )
        else:
            send_message(
    ...
    if stripped.startswith("/start"):
        ...
        send_message(
            ...
            add_emoji_prefix(
                ...
                "В 18:00 я мягко напомню, если ты ничего не написала, а в 21:00 пришлю небольшой отчёт за день.\n\n"
    ...
    if is_anxiety_message(cleaned):
        ...
        # ритуал не отправляет отдельного сообщения
        maybe_offer_ritual(chat_id)
        return
...
def send_daily_report_for_user(chat_id: int):
    ...
    # фильтруем записи, оставляя только подходящие для отчёта
    filtered_joys = [(created_at, text) for created_at, text in joys if is_joy_suitable_for_report(text)]

    if not filtered_joys:
        send_message(
            chat_id,
            add_emoji_prefix(
                "Сегодня у меня нет записанных радостей, которые подошли бы для отчёта.\n"
    ...
    lines = []
    for created_at, text in filtered_joys:
        try:
            dt = datetime.fromisoformat(created_at)
